...
// Symbol used to mark synthetic 1-byte pointer-only buffers created by
// WrapPointer(ptr, 0) for non-NULL ptr. The N-API spec doesn't preserve the
// data pointer for 0-length TypedArrays, so WrapPointer uses length=1 as a
// workaround. However, the single byte at the sentinel address (e.g.
// RTLD_NEXT = 0xffffffffffffffff) is not readable, so refinspect must not
// attempt to read it. This symbol is set by the native WrapPointer to signal
// that the buffer holds only a pointer value, not inspectable data bytes.
const kPointerOnly = Symbol.for('nodejs.ref-napi.pointer_only');
exports.kPointerOnly = kPointerOnly;
...
function overwriteInspect (inspect) {
  if (inspect.name === 'refinspect') {
    ...
    return function refinspect () {
      ...
      // Pointer-only buffers are synthetic 1-byte buffers created by
      // WrapPointer(ptr, 0) for non-NULL ptr. Their backing byte may be at an
      // inaccessible sentinel address (e.g. RTLD_NEXT = 0xffffffffffffffff).
      // Reading that byte via the original inspect would cause a SIGSEGV.
      if (this[kPointerOnly]) {
        return `<Buffer@0x${this.hexAddress()}>`;
      }
